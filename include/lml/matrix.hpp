...
struct Matrix {
	...
	inline float &operator()(size_t row, size_t col) { return data[row * cols + col]; }

	inline float operator()(size_t row, size_t col) const { return data[row * cols + col]; }
...
inline void assert_dim_equal(const Matrix &A, const Matrix &B) { assert(A.rows == B.rows && A.cols == B.cols); };

inline void assert_dim(const Matrix &A, const Matrix &B) { assert(A.cols == B.rows); };
} // namespace LML